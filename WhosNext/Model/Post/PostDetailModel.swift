...
struct PostDetailModel: Codable, Hashable, Equatable {
    var success: Bool?
    var statusCode: Int?
    var message: String?
...
struct PostDetailData: Codable, Hashable, Equatable {
...
extension PostDetailModel {
    /// `api call` for post detail
    static func postDetails(params: [String: Any], success: @escaping (PostDetailModel?, String) -> Void, failure: @escaping (String) -> Void) -> Void {
        Indicator.show()
        ...
        APIManager.makeRequest(with: Constant.ServerAPI.kPostDetail, method: .post, parameter: params, success: { response -> Void in
            ...
            do {
                var updatedPostDetailModel = try JSONDecoder().decode(PostDetailModel.self, from: data)

                if var post = updatedPostDetailModel.data {
                            ...
                            AWSS3Manager.shared.getSignedUrl(key: intro_video_path, withSuccess: { introVideo in
                                ...
                                AWSS3Manager.shared.getSignedUrl(key: intro_video_thumb_path, withSuccess: { introVideoThumb in
                                    ...
                                    if post.postComments!.count > 0 {
                                        ...
                                        updatedPostDetailModel.data = post
                                    } else {
                                        updatedPostDetailModel.data = post
                                    }
                ...
                success(updatedPostDetailModel, updatedPostDetailModel.message ?? "")
            } catch let error {
