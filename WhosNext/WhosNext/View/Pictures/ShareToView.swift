...
struct PeopleList: Identifiable {
    var id: Int
    var fullName: String
    var name: String
}
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    HStack {
                        ...
                        Text(IdentifiableKeys.NavigationbarTitles.kShareTo)
    ...
    func followersView() -> some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading) {
                ...
                CommonButton(title: IdentifiableKeys.Buttons.kShare, cornerradius: 0) {
                    if self.shareToVM.isUploading == false {
                        var postId = ""
                        ...
                        if self.postDetailsModel != nil {
                            ...
                            if self.shareToVM.postType == 1 {
                                self.shareToVM.updatePostWithImage(postID: postId, postType: 1, postSubType: self.currentTab == 0 ? 1 : 2, postCaption: self.text ?? "",
                                                                   taggedSelectedPeople: tagPeople, taggedSelectedPeopleInGroupVideo: "", deleteImageName: String(deleteFileName)) {
                                    self.presentationMode.wrappedValue.dismiss()
                                }
                            } else if self.shareToVM.postType == 2 {
                                self.shareToVM.updatePostWithVideo(postID: postId, postType: 2, postSubType: self.currentTab == 0 ? 1 : 2, postCaption: self.text ?? "",
                                                                   taggedSelectedPeople: tagPeople, deleteVideoName: String(deleteFileName), deleteVideoThumbName: String(deleteFileThumbName)) {
                                    self.presentationMode.wrappedValue.dismiss()
                                }
                            }
                        } else {
                            if self.isVideo == false {
                                self.shareToVM.createPostWithImage(postID: postId, postType: 1, postSubType: self.currentTab == 0 ? 1 : 2, postCaption: self.text ?? "",
                                                                   taggedSelectedPeople: tagPeople) {
                                    self.viewRouter.currentView = .Home
                                    NavigationUtil.popToRootView()
                                }
                            } else {
                                self.shareToVM.createPostWithVideo(postID: postId, postType: 2, postSubType: self.currentTab == 0 ? 1 : 2, postCaption: self.text ?? "",
                                                                   taggedSelectedPeople: tagPeople, taggedSelectedPeopleInGroupVideo: "") {
                                    self.viewRouter.currentView = .Home
                                    NavigationUtil.popToRootView()
                                }
                            }
